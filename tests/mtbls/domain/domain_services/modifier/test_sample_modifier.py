...
class TestRemoveTrailingAndPrefixSpaces:
    @pytest.mark.asyncio
    async def test_modifier_remove_trailing_01(self, sample_modifier: SampleFileModifier):
        ...
        sample_file.table.data[header_def.column_name] = sample_file.table.data[header_column_name]
    ...
    @pytest.mark.asyncio
    async def test_modifier_remove_trailing_02(self, sample_modifier: SampleFileModifier):
...
class TestUpdateOntologyColumns:
    @pytest.mark.asyncio
    async def test_update_ontology_columns_01(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_modifier.update_ontology_columns()
        assert len(sample_modifier.update_logs) == 0

    @pytest.mark.asyncio
    async def test_update_ontology_columns_02(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
    ...
    @pytest.mark.asyncio
    async def test_update_ontology_columns_03(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        sample_table = sample_modifier.model.samples[sample_file_name].table
        sample_table.data["Characteristics[Organism]"][0] = "homo sapiens"
        ...
        assert sample_table.data["Term Accession Number"][0] == "http://purl.obolibrary.org/obo/NCBITaxon_9606"
        assert len(sample_modifier.update_logs) == 1

    @pytest.mark.asyncio
    async def test_update_ontology_columns_04(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        sample_table = sample_modifier.model.samples[sample_file_name].table
        sample_table.data["Characteristics[Organism]"][0] = ""
        sample_table.data["Term Source REF"][0] = ""
        sample_table.data["Term Accession Number"][0] = "http://purl.obolibrary.org/obo/NCBITaxon_9606"
        sample_modifier.update_ontology_columns()
        assert sample_table.data["Characteristics[Organism]"][0] == "Homo sapiens"
        assert sample_table.data["Term Source REF"][0] == "NCBITAXON"
        assert sample_table.data["Term Accession Number"][0] == "http://purl.obolibrary.org/obo/NCBITaxon_9606"
    ...
    @pytest.mark.asyncio
    async def test_update_ontology_columns_07(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        sample_table = sample_modifier.model.samples[sample_file_name].table
        sample_table.data["Characteristics[Organism]"][0] = ""
...
class TestUpdateSampleFactorValues:
    @pytest.mark.asyncio
    async def test_update_sample_factor_values_01(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
    ...
    @pytest.mark.asyncio
    async def test_update_sample_factor_values_02(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        sample_table = sample_modifier.model.samples[sample_file_name].table
        for header in sample_table.headers:
            if header.column_header == "Factor Value[Metabolic syndrome]":
                ...
                sample_table.columns[header.column_index] = "Factor Value[metabolic Syndrome]"
                sample_table.data["Factor Value[metabolic Syndrome]"] = sample_table.data[
...
class TestAddFactorValueColumns:
    ...
    @pytest.mark.asyncio
    async def test_add_factor_value_columns_01(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        ...
        assert len(sample_modifier.new_header_actions[sample_file_name][0].columns) == expected_columns_count
        assert len(sample_modifier.update_logs) == expected_logs_count

    @pytest.mark.asyncio
    async def test_add_factor_value_columns_02(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        study = sample_modifier.model.investigation.studies[0]
        factors = study.study_factors.factors
    ...
    @pytest.mark.asyncio
    async def test_add_factor_value_columns_03(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        study = sample_modifier.model.investigation.studies[0]
        factors = study.study_factors.factors
        factors.append(Factor(name="Test", type=OntologyAnnotation(term="mm", term_source_ref="UO")))
        sample_modifier.add_factor_value_columns()
        assert sample_modifier.new_header_actions
        assert sample_file_name in sample_modifier.new_header_actions
        expected_logs_count = 1
    ...
    @pytest.mark.asyncio
    async def test_add_factor_value_columns_04(self, sample_modifier: SampleFileModifier):
        sample_modifier.max_row_number_limit = 3
        sample_file_name = sample_modifier.model.investigation.studies[0].file_name
        study = sample_modifier.model.investigation.studies[0]
        factors = study.study_factors.factors
