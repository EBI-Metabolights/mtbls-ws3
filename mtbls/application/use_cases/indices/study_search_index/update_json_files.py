...
from mtbls.application.use_cases.indices.study_search_index import schemas
from mtbls.application.use_cases.indices.study_search_index.assay_classifier import (
    AssayClassifier,
    AssayTechnique,
)
...
FUNDER_REF_ID_COMMENT_NAMES = {"FundRef ID", "Funder ROR ID"}
...
async def get_studies(
    ...
    target_study_status_list: list[StudyStatus],
    ...
    studies: list[StudyOutput] = await read_study_repository.get_studies(
        filters=[
            EntityFilter(
                key="status", operand=FilterOperand.IN, value=target_study_status_list
            )
        ],
...
async def update_study_metadata_json_files(
    ...
    force_reindex_studies: None | list[str] = None,
    target_study_status_list: None | list[StudyStatus] = None,
):
    if not force_reindex_studies:
        force_reindex_studies = []
    if not temp_folder:
    ...
    if not target_study_status_list:
        target_study_status_list = [StudyStatus.PUBLIC]
    study_map = await get_studies(
        study_read_repository, target_study_status_list=target_study_status_list
    )
    ...
    for study_id in studies:
        try:
            ...
            if not reindex_all and file_exist and study_id not in force_reindex_studies:
                logger.debug(
