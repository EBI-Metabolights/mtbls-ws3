...
from mtbls.infrastructure.ontology_search.bioportal.bioportal_configuration import (
    BioPortalConfiguration,
)
from mtbls.infrastructure.ontology_search.bioportal.schemas import (
    BioportalSearchResultItem,
)

logger = logging.getLogger(__name__)


class BioPortalOntologySearchService(OntologySearchService):
    def __init__(
        self,
        http_client: HttpClient,
        config: None | BioPortalConfiguration | dict[str, Any] = None,
    ):
        self.http_client = http_client
        self.config = config
        if not self.config:
            self.config = BioPortalConfiguration()
    ...
    async def search_by_validation_type(
        ...
        if validation_type == OntologyValidationType.ANY_ONTOLOGY_TERM:
            ...
            responses = []
            results = []
            for term in rule.allowed_parent_ontology_terms.parents:
                response, result = await self.search_term(
                    keyword,
                    ontology_filter=[term.term_source_ref],
                    parents=[term.term_accession_number],
                    page=page,
                    size=size,
                    exact_match_only=exact_match,
                )
            ...
            result = results
            response = responses[0] if responses else HttpResponse()
    ...
    async def search_term(
        ...
        url = f"{self.config.origin_url}/search"

        params = {
            "q": keyword,
            "require_exact_match": "true" if exact_match_only else "false",
            "include": "prefLabel,synonym,definition,notation,cui,semanticType",
            "pagesize": size,
            "format": "json",
        }

        headers = {"Authorization": f"apikey token={self.config.api_token}"}
        ...
        if parents:
            ...
            params.update({"ontology": ",".join([x for x in ontology_filter if x])})
        elif ontology_filter:
            params.update({"ontologies": ",".join([x for x in ontology_filter if x])})
        ...
        if result.error:
            return result, []
        matches = result.json_data.get("collection", {}) if result.json_data else {}

        if not matches:
            return result, []
        hits = [
            BioportalSearchResultItem.model_validate(x).convert_to_ontology_term_hit(
