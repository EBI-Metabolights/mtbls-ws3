...
class MongoDbValidationOverrideService(ValidationOverrideService):
    ...
    async def _initiate_validation_override(self, resource_id: str) -> Union[None, ValidationOverrideFileObject]:
        filters = [
            EntityFilter(key="resourceId", value=resource_id),
        ]
        files = await self.validation_override_repository.find(query_options=QueryOptions(filters=filters))
        ...
        if not overrides_model:
            ...
            await self.validation_override_repository.object_created(
                self.get_study_object(id_=overrides.id_, resource_id=resource_id, created_at=now)
            )
    ...
    async def save_validation_overrides(self, resource_id: str, validation_overrides: ValidationOverrideList) -> bool:
        ...
        await self.validation_override_repository.object_updated(
            self.get_study_object(id_=updated_object.id_, resource_id=resource_id, updated_at=now)
        )
