        ...
        if isinstance(item, list):
            for comment in item:
                ...
                if isinstance(value, list):
                    comments.extend([CommentItem(name=name, value=str(x)) for x in value])
...
class CommentedOntologySourceReferenceItem(CommentedItem, OntologySourceReferenceItem): ...
...
class ProtocolItem(CamelCaseModel):
    ...
    version: Annotated[
        str,
        Field(description="An identifier for the version to ensure protocol tracking.."),
    ] = ""
...
class PublicationItem(CamelCaseModel):
    ...
    title: Annotated[
        str,
        Field(description="The title of publication associated with the investigation."),
    ] = ""
    status: Annotated[
        OntologyItem,
        Field(
...
class InvestigationItem(CommentedItem):
    ...
    @classmethod
    def validate_ontology_source_references(cls, value) -> list[CommentedOntologySourceReferenceItem]:
        if not value:
            return []
    ...
    def validate_model(cls, v: Any, handler) -> Investigation:
        ...
        if isinstance(v, InvestigationItem):
            ...
            item.investigation_contacts = InvestigationContacts(
                people=[Person.model_validate(x, from_attributes=True) for x in source.contacts],
                comments=build_comments(source.contacts),
            )

            item.investigation_publications = InvestigationPublications(
                publications=[Publication.model_validate(x, from_attributes=True) for x in source.publications],
            ...
            for idx, study_item in enumerate(source.studies):
                ...
                study.study_assays = StudyAssays(
                    assays=[Assay.model_validate(x, from_attributes=True) for x in study_item.assays],
                    comments=build_comments(study_item.assays),
                )
                study.study_factors = StudyFactors(
                    factors=[Factor.model_validate(x, from_attributes=True) for x in study_item.factors],
                    comments=build_comments(study_item.factors),
                )
                study.study_protocols = StudyProtocols(
                    protocols=[Protocol.model_validate(x, from_attributes=True) for x in study_item.protocols],
                ...
                study.study_contacts = StudyContacts(
                    people=[Person.model_validate(x, from_attributes=True) for x in study_item.contacts],
                    comments=build_comments(study_item.contacts),
                )
                study.study_publications = StudyPublications(
                    publications=[Publication.model_validate(x, from_attributes=True) for x in study_item.publications],
...
def get_subitems(
    ...
    values: list[CommentedItem] = [model_class.model_validate(x, from_attributes=True) for x in source_list]
    ...
    for idx, x in enumerate(values):
        for comment in source_section.comments:
            if isinstance(comment.value, str):
                if idx == 0:
                    x.comments.append(CommentItem(name=comment.name, value=comment.value))
